...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='madalon', help='Specify dataset. One of "cifar10", "fashionmnist",'
                                                             '"madalon",  or "mnist"')
    ...
    for epoch in range(n_epochs):
        ...
        for j in range(data.x_train.shape[0] // data.batch_size):
            ...
            for i in range(2,5):
                ...
                dw = update[index-1][0]
                delta = update[index-1][1]
                # if index not in da:
                #     da[index] = - model.learning_rate * (np.mean(dw, axis=0).mean() + delta.mean()) * (model.w[index-1].data.mean())/slope
                # else:
                #     da[index] = model.momentum * da[index] - model.learning_rate * (np.mean(dw,axis=0).mean()+delta.mean()) * (model.w[index-1].data.mean())/slope
                # if index not in da:
                #     da[index] = - model.learning_rate * (np.mean(dw, axis=0).mean() )
                # else:
        ...
        save_slope[2].append(model.activations[2].slope)
        save_slope[3].append(model.activations[3].slope)
        save_slope[4].append(model.activations[4].slope)
        ...
        print("epoch : ", epoch, "acc : ", acc, 'EGF : ', EGF)
        print(save_slope[2][epoch],'  ',save_slope[3][epoch],'  ',save_slope[4][epoch])

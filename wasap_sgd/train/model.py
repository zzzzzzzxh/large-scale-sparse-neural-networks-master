...
def backpropagation_updates_numpy(a, delta, rows, cols, out):
    # this function change nothing, the out should update dw.data which means w*error
    #backpropagation_updates_numpy(a[self.n_layers - 1], delta, dw.row, dw.col, dw.data)
    # a[4-1]32x100 delta 32x400
...
class SETMPIModel(object):
    ...
    def _back_prop(self, z, a, masks, y_true):
        """
        The input dicts keys represent the layers of the net. batch_size*neural numbers
        a = { 1: x,
              2: f(w1(x) + b1)
              3: f(w2(a2) + b2)
              4: f(w3(a3) + b3)
        ...
        for i in reversed(range(2, self.n_layers)):
            ...
            # compute backpropagation updates ??? cant work
